...
\text{where } $p\in \mathbb{R}^{n_{p}}$ \text{ is an uncertain parameter vector}


\onslide<2->
%\begin{equation*}
\text{where } $ p\in\mathbb{P}_{box}=\left\{\left. p\in\mathbb{R}^{n_{p}} \right| p_{lower}\leq p \leq p_{upper}\right\}$
%\end{equation*}
\end{overprint}
\end{block}
