...
figure
subplot(2,2,1)
plot(0:h:tf,zMin(1:2:2*n-1),'LineWidth',2)
title('Solution position y')
xlabel('Time [s]')
ylabel('Path [m]')
subplot(2,2,2)
plot(0:h:tf,zMin(2:2:2*n),'LineWidth',2)
title('Solution velocity v')
xlabel('Time [s]')
ylabel(' v [m/s]')
subplot(2,2,3)
stairs(0:h:tf,zMin(2*n+1:2:4*n-1),'LineWidth',2)
title('Solution acceleration torque Mwh')
xlabel('Time [s]')
ylabel(' M_{wh} [Nm]')
subplot(2,2,4)
stairs(0:h:tf,cCCP.R.*zMin(2*n+2:2:4*n),'LineWidth',2)
